...
.tok-composer {
  ...
  &__field {
    ...
    // Фокус приходит на `<input>`, а видимая «таблетка» — это поле вокруг него.
    // Поэтому индикатор рисуется на родителе: обводка вокруг голого текстового
    // бокса шла бы прямоугольником внутри скруглённого поля.
    // `:focus-within`, а не `:focus-visible`: у `<input>` фокус с клавиатуры и
    // мышью равнозначен, и в обоих случаях человек должен видеть, куда печатает.
    &:focus-within {
