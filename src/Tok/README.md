...
Tok/
  ...
  theme/tokens.js          ← цвета графиков (единственное, что осталось от темы)
```
...
| `index.js` | публичный вход: `Tok` (он же default), `tokChartTokens`, константы контракта |
...
| `theme/tokens.js` | цвета графиков для amCharts; **единственное** место с hex внутри папки |
...
2. Убедиться, что `@tne-ui/core` объявляет переменные `--v-tok-*`. Полный список
   имён — в `src/demo/styles/tok-vars.scss` этого репозитория; в самой папке они
   встречаются россыпью, по месту употребления: `grep -o 'var(--v-tok-[a-z0-9-]*)'`.
   **Суффикса `-base` быть не должно.**
3. Настраивать sass-loader не нужно. Стили компонентов ни от чего не зависят:
   ни функций, ни миксинов, ни SCSS-переменных в папке не осталось — цвет
   написан как `var(--v-tok-*)`, всё остальное числами прямо в правиле
   ([ADR-0010](../../docs/adr/0010-stili-bez-kontrakta-tema-cherez-vuetify.md)).
...
Сборка в этом репозитории: алиас `@` → `src` и `sass`. Ничего подмешивать
в SFC через `css.loaderOptions.scss.additionalData` для Тока не нужно
...
Изолированный просмотр, песочница, чужой стенд — там переменные объявляет сам стенд.
Раскладывать их из папки Тока больше нечем и незачем: своего механизма темы у него
нет, тему переключает хост. Как это устроено на стенде этого репозитория — см.
`src/demo/styles/tok-vars.scss`: цвета лежат в палитре Vuetify, а Vuetify их и
переписывает при смене темы.
...
Цвет пишется в стилях компонента целиком: `background-color: var(--v-tok-surface);`.
Ни функции, ни миксина между стилем и переменной нет. Переменные объявляет
`@tne-ui/core` и переключает вместе с темой Vuetify — своего механизма темы
у папки нет ([ADR-0010](../../docs/adr/0010-stili-bez-kontrakta-tema-cherez-vuetify.md)).

Hex внутри папки допустим **только** в `theme/tokens.js`, и остались там ровно
цвета графиков. Причина одна: `services/charts/palette.js` — amCharts рисует SVG
из JavaScript и строку `var(--v-…)` не понимает. Палитра достаёт **вычисленное**
значение переменной из стилей контейнера, а если переменных нет (jsdom в тестах,
изолированный рендер) — берёт значение прямо из `tokens.js`. Дефолтная палитра
amCharts не используется нигде.

## Точки замены при переносе

Ни одного `v-*`-компонента внутри папки нет: Vuetify нужен хосту — он переключает
тему, — а не самому Току, который читает только готовые переменные. Поэтому «слой-обёртка» над примитивами
